...
sys.path.insert(0, '..')
sys.path.insert(0, '.')
import bacula_tools, bacula_tools.parser_support
...
class overall_parser_tests(unittest.TestCase):
    ...
    def test_import_fd_conf(self):
        retval = bacula_tools.parser_support.parser(open('/etc/bacula/bacula-fd.conf').read())
        self.assertIsInstance(retval, list)
        return

    def test_import_sd_conf(self):
        retval = bacula_tools.parser_support.parser(open('/etc/bacula/bacula-sd.conf').read())
        self.assertIsInstance(retval, list)
        return

    def test_import_dir_conf(self):
        retval = bacula_tools.parser_support.parser(open('/etc/bacula/bacula-dir.conf').read())
        self.assertIsInstance(retval, list)
        return
