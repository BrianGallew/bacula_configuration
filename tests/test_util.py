...
class hostname_mangler_tests(unittest.TestCase):
    def test_exmample_com(self):
        self.assertEquals(bacula_tools.hostname_mangler('www.example.com'), 'www')
        return
    def test_exmample_net(self):
        self.assertEquals(bacula_tools.hostname_mangler('www.example.net'), 'www')
        return
...
class plist_tests(unittest.TestCase):
    ...
    def test_three_items(self):
        ...
        obj = bacula_tools.PList(ts)
        res = obj[:]
        self.assertEquals(['one two three','one twothree','onetwo three','onetwothree', ], res)
        return
    def test_four_items(self):
        ts = 'one two three four'
        obj = bacula_tools.PList(ts)
        res = obj[:]
        self.assertEquals(['one two three four','one two threefour','one twothree four','one twothreefour','onetwo three four','onetwo threefour','onetwothree four','onetwothreefour', ], res)
        return
...
class database_storage_tests(unittest.TestCase):
    def setUp(self):
        ...
        self.bc.connect(database = 'test', host = 'localhost', user = 'test', passwd = 'test')
        foo = sys.stderr
    ...
    def tearDown(self):
        ...
        for s in sql:
            s = s.strip()
            if not s: continue
            if not 'DROP TABLE' in s: continue
            #row = self.bc.do_sql(s)
        sys.stderr = foo
        return
    ...
    def test_password_store_valid(self):
        ...
        p.password=PWORD
        p.store()
        result = self.bc.do_sql('''SELECT password FROM pwords WHERE obj_id=%s and obj_type=%s and director_id=%s and director_type=%s''', (self.client[bacula_tools.ID], self.client.IDTAG, self.director[bacula_tools.ID], self.director.IDTAG))
        self.bc.do_sql('delete from pwords')
    ...
    def test_password_load(self):
        self.bc.do_sql('delete from pwords')
        self.bc.do_sql('''INSERT INTO pwords (obj_id, obj_type, director_id, director_type, password) values (%s, %s, %s, %s, %s)''', (self.client[bacula_tools.ID], self.client.IDTAG, self.director[bacula_tools.ID], self.director.IDTAG, 'fred'))
        p = bacula_tools.PasswordStore(self.client, self.director)
        self.bc.do_sql('delete from pwords')
    ...
    def test_default_director_by_name(self):
        bacula_tools.default_director(self.client, self.director[bacula_tools.NAME])
        self.assertEquals(len(self.bc.do_sql('select * from pwords')), 1)
        self.bc.do_sql('delete from pwords')
        return
...
class guess_os_tests(unittest.TestCase):
    def test_default(self):
        ...
        return
    def test_agent(self):
        os.environ['HTTP_USER_AGENT'] = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/32.0.1700.102 Safari/537.36'
