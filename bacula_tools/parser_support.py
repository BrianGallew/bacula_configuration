...
def client_parse_string(self, string):
    ...
    da_ip = np(('IPv4','IPv6','IP'), nestedExpr('{','}', OneOrMore(da_addr).setParseAction(lambda x,y,z: ' ; '.join(z)))).setParseAction(handle_ip)
    da_addresses = np(('fd addresses', FDADDRESSES), nestedExpr('{','}', OneOrMore(da_ip)), handle_fdaddr)
...
def job_parse_string(self, string):
    ...
    gr_line = gr_line | np(PList('Run Before Job'), gr_stripped_string, action=self._parse_script(runsonclient=0, runswhen='Before'))
    gr_line = gr_line | np(PList('Run After Job'), gr_stripped_string, action=self._parse_script(runsonclient=0, runswhen='After'))
    ...
    gr_line = gr_line | np(PList('Client Run Before Job'), gr_stripped_string, action=self._parse_script(runswhen='Before'))
    gr_line = gr_line | np(PList('Client Run After Job'), gr_stripped_string, action=self._parse_script(runswhen='After'))

    # This is a complicated one
    gr_script_parts = np(('Command',), gr_stripped_string, action=nullDebugAction)
    gr_script_parts = gr_script_parts | np((CONSOLE,), gr_stripped_string, action=nullDebugAction)
    gr_script_parts = gr_script_parts | np(PList('Runs When'), action=nullDebugAction)
    gr_script_parts = gr_script_parts | np(PList('Runs On Success'), gr_yn, action=nullDebugAction)
    gr_script_parts = gr_script_parts | np(PList('Runs On Failure'), gr_yn, action=nullDebugAction)
    gr_script_parts = gr_script_parts | np(PList('Runs On Client'), gr_yn, action=nullDebugAction)
    gr_script_parts = gr_script_parts | np(PList('Fail Job On Error'), gr_yn, action=nullDebugAction)
